...
describe(scriptName + ' script', function () {
  ...
  var today = new Date(),
    ...
    diff3Months =  Math.round((today.getTime() - ThreeMonthsAgoDay.getTime())  / 86400000);
  
  console.log(diff3Months-1);
  ...
  describe('tmp action adder', function () {
    ...
    it('should add new script ' + scriptSetCounterName + ' that set counter ' + testCounterName + ' to '+(diff3Months-1)+' at ' + URL, function () {

      runner.one()
        .action({
          name: scriptSetCounterName,
          code: '$self.counters().add("' + testCounterName + '", '+(diff3Months-1)+');',
  ...
  describe('[runner] logic', function () {
    ...
    it('send 2 events [(#1- ' + scriptSetCounterName + ' that add counter ' + testCounterName + ' and set it to 90), (#2- ' + scriptIncName + ' that increase counter ' + testCounterName + ')] and earn achievement regularUser4', function () {
      ...
      runner.one('invoke script')
        ...
        .finalTest(function () {
          ...
          restReq({
            ...
            expect(_res.usage).toBe(diff3Months);
