...
var scriptName = 'freq_buy_descalingCapsule';
...
describe(scriptName + ' script', function () {
  ...
  var scriptIncName = utils.getActionName(),
    ...
    testCounterName = 'buyProduct_descalingCapsule';

  describe('tmp action adder', function () {

    it('should add new script ' + scriptSetCounterName + ' that set counter ' + testCounterName + ' to 7 at ' + URL, function () {

      runner.one()
        .action({
          name: scriptSetCounterName,
          code: '$self.counters().add("' + testCounterName + '", 7);',
  ...
  describe('achievements', function () {
    ...
    it('should add achievements buyProduct_descalingCapsule_{level}', function () {

      runner.one()
        .achievement({name: 'descale_sheriff_1'})
        .achievement({name: 'descale_sheriff_2'})
        .achievement({name: 'descale_sheriff_3'})
        .run();
  ...
  describe('[runner] logic', function () {
    ...
    it('send 2 events [(#1- ' + scriptSetCounterName + ' that add counter ' + testCounterName + ' and set it to 7), (#2- ' + scriptIncName + ' that increase counter ' + testCounterName + ')] and earn achievements buyProduct_descalingCapsule_amountXX', function () {
      ...
      runner.one('invoke script')
        ...
        .finalTest(function () {
          ...
          restReq({
            ...
            expect(_res.buyProduct_descalingCapsule).toBe(8);
            ...
            expect(keys).toContain('descale_sheriff_1');
            expect(keys).toContain('descale_sheriff_2');
            expect(keys).toContain('descale_sheriff_3');
