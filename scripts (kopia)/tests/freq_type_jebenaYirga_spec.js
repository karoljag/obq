...
var scriptName = 'freq_type_jebenaYirga';
...
describe(scriptName + ' script', function () {
  ...
  var scriptIncName = utils.getActionName(),
    ...
    testCounterName = 'coffeeType_jebenaYirga';
  ...
  describe('achievements', function () {
    ...
    it('should add achievements coffeeType_jebenaYirga_{level}', function () {

      runner.one()
        .achievement({name: 'jebena_yirga_tester'})
        .achievement({name: 'jebena_yirga_fan_1'})
        .achievement({name: 'jebena_yirga_fan_2'})
        .achievement({name: 'jebena_yirga_fan_3'})
        .run();
  ...
  describe('[runner] logic', function () {
    ...
    it('send 2 events [(#1- ' + scriptSetCounterName + ' that add counter ' + testCounterName + ' and set it to 149), (#2- ' + scriptIncName + ' that increase counter ' + testCounterName + ')] and earn achievements coffeeType_jebenaYirga_amountXX', function () {
      ...
      runner.one('invoke script')
        ...
        .finalTest(function () {
          ...
          restReq({
            ...
            expect(_res.coffeeType_jebenaYirga).toBe(150);
            ...
            expect(keys).toContain('jebena_yirga_tester');
            expect(keys).toContain('jebena_yirga_fan_1');
            expect(keys).toContain('jebena_yirga_fan_2');
            expect(keys).toContain('jebena_yirga_fan_3');
  ...
  var userName2 = utils.getUsername();
  ...
  describe('[runner] logic', function () {
    ...
    it('one time', function () {
      ...
      runner.one('invoke script')
        ...
        .events([eventIncCounter])
        .finalTest(function () {

          restReq({
            url: URL + '/users/' + userName2 + '/counters',
            method: 'get'
          }).then(function (res) {
            ...
            expect(_res.coffeeType_jebenaYirga).toBe(1);

            return restReq({
              url: URL + '/users/' + userName2 + '/achievements?limit=999',
              method: 'get'
            })
