...
var scriptName = 'freq_type_volcanesAntigua';
...
describe(scriptName + ' script', function () {
  ...
  var scriptIncName = utils.getActionName(),
    ...
    testCounterName = 'coffeeType_volcanesAntigua';
  ...
  describe('achievements', function () {
    ...
    it('should add achievements coffeeType_volcanesAntigua_{level}', function () {

      runner.one()
        .achievement({name: 'volcanes_antigua_tester'})
        .achievement({name: 'volcanes_antigua_fan_1'})
        .achievement({name: 'volcanes_antigua_fan_2'})
        .achievement({name: 'volcanes_antigua_fan_3'})
        .run();
  ...
  describe('[runner] logic', function () {
    ...
    it('send 2 events [(#1- ' + scriptSetCounterName + ' that add counter ' + testCounterName + ' and set it to 149), (#2- ' + scriptIncName + ' that increase counter ' + testCounterName + ')] and earn achievements coffeeType_volcanesAntigua_amountXX', function () {
      ...
      runner.one('invoke script')
        ...
        .finalTest(function () {
          ...
          restReq({
            ...
            expect(_res.coffeeType_volcanesAntigua).toBe(150);
            ...
            expect(keys).toContain('volcanes_antigua_tester');
            expect(keys).toContain('volcanes_antigua_fan_1');
            expect(keys).toContain('volcanes_antigua_fan_2');
            expect(keys).toContain('volcanes_antigua_fan_3');
